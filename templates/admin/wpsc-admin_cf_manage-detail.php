	...
	<div id="col-left">
		<div class="col-wrap">
			<div class="form-wrap">
				...
				<form method="post" action="<?php echo add_query_arg( array( 'post_type' => 'wpsc-product', 'page' => 'wpsc_cf' ), 'edit.php' ); ?>" class="validate">

					<div class="form-field form-required">
						<label><?php _e( 'Name', 'wpsc_cf' ); ?><span class="description"> (<?php _e( 'required', 'wpsc_cf' ); ?>)</span></label>
						<input type="text" name="custom-field-name" value="<?php if( isset( $field['name'] ) ) echo stripslashes( $field['name'] ); ?>" />
					...
					<div class="form-field form-required">
						...
						<input type="text" name="custom-field-slug" value="<?php if( isset( $field['slug'] ) ) echo $field['slug']; ?>" />
...
<?php if( $field['type'] == 'dropdown' || $field['type'] == 'checkbox' || $field['type'] == 'radio' ) { ?>
					...
					<div class="form-field">
						...
						<p class="description"><?php _e( 'Separate each option with a \'|\' (pipe) character.<br />For instance, \'Option 1|Option 2\' will display 2 options on the Add/Edit&nbsp;Products page.', 'wpsc_cf' ); ?></p>
					</div>
<?php } ?>

					<div class="form-field">
						<label><?php _e( 'Order', 'wpsc_cf' ); ?></label>
						<input type="text" name="custom-field-order" value="<?php if( isset( $field['order'] ) && $field['order'] ) echo $field['order']; else echo '0'; ?>" size="4" />
					</div>

					<div class="form-field">
						<label><?php _e( 'Prefix', 'wpsc_cf' ); ?></label>
						<input type="text" name="custom-field-prefix" value="<?php echo ( isset( $field['prefix'] ) && $field['prefix'] ) ? stripslashes( $field['prefix'] ) : '' ?>" size="4" class="text" />
						<p class="description"><?php _e( 'A prefix to add when shown on your site', 'wpsc_cf' ); ?>.</p>
					</div>

					<div class="form-field">
						<label><?php _e( 'Suffix', 'wpsc_cf' ); ?></label>
						<input type="text" name="custom-field-suffix" value="<?php echo ( isset( $field['suffix'] ) && $field['suffix'] ) ? stripslashes( $field['suffix'] ) : '' ?>" size="4" />
						<p class="description"><?php _e( 'A suffix to add when shown on your site', 'wpsc_cf' ); ?>.</p>
					</div>
					
					<div class="form-field">
						<label>
							<input type="checkbox" style="width:13px;" name="custom-field-show-name"<?php if( $action == 'edit' ) { checked( $field['show_name'], 'on' ); } else { checked( false, false ); } ?> />
					...
					<div class="form-field">
						...
						<textarea name="custom-field-description" rows="5" cols="40"><?php echo ( isset( $field['description'] ) && $field['description'] ) ? stripslashes( $field['description'] ) : ''; ?></textarea>
						<p class="description"><?php _e( 'The description is not prominent by default; however, some themes may show it', 'wpsc_cf' ); ?>.</p>
					</div>

<?php if( $action == 'new' ) { ?>
					<p class="submit">
						<input type="submit" id="submit" value="<?php _e( 'Add New Attribute', 'wpsc_cf' ); ?>" class="button-primary" />
					</p>
