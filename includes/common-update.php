...
function wpsc_cf_plugin_update_check( $checked_data ) {

	global $wpsc_cf, $api_url;

	$plugin_slug = $wpsc_cf['dirname'];
	$plugin_path = $wpsc_cf['relpath'];
...
add_filter( 'pre_set_site_transient_update_plugins', 'wpsc_cf_plugin_update_check' );

function wpsc_cf_plugin_update_call( $def, $action, $args ) {
...
add_filter( 'plugins_api', 'wpsc_cf_plugin_update_call', 10, 3 );
?>